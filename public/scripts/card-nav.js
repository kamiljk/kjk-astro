...
function initCardNavigation() {
  ...
  function closeHeroCard() {
    ...
    setTimeout(() => {
      ...
      if (heroMarkdownContainer) {
        ...
        heroMarkdownContainer.innerHTML = '<div class="loading-indicator" style="display: none;"><div class="loading-spinner"></div><p>Loading...</p></div>';
        heroMarkdownContainer.classList.remove('has-post-content', 'has-game-info');
  ...
  window.activateHeroCard = (title, contentHTML = null, isGameInfo = false) => {
    ...
    const heroContentArea = heroCard.querySelector('.hero-card-content');
    const heroIframe = heroCard.querySelector('.hero-iframe');
    const loadingIndicator = heroMarkdownContainer?.querySelector('.loading-indicator');
    ...
    if (contentHTML) {
      ...
      if (heroMarkdownContainer) {
        ...
        heroMarkdownContainer.classList.add(isGameInfo ? 'has-game-info' : 'has-post-content');
        heroMarkdownContainer.classList.remove(isGameInfo ? 'has-post-content' : 'has-game-info');
      }
    } else {
      ...
      if (heroMarkdownContainer) {
         ...
         heroMarkdownContainer.innerHTML = '<div class="loading-indicator" style="display: flex;"><div class="loading-spinner"></div><p>Loading...</p></div>';
         heroMarkdownContainer.classList.remove('has-post-content', 'has-game-info');
      }
    }
  ...
  document.body.addEventListener('click', async (e) => {
    ...
    if (!title || !slug || typeof window.activateHeroCard !== 'function') {
      console.error("Learn More button missing data or activateHeroCard not found.");
      return;
    }
    ...
    if (isGame) {
      ...
      const fetchUrl = learnButton.getAttribute('data-link') || learnButton.getAttribute('href');
      if (!fetchUrl) {
         console.error("Missing fetch URL for non-game card:", title);
         window.activateHeroCard(title, '<p class="error-message">Error: Could not find content link.</p>', false);
         return;
      }

      console.log(`[card-nav.js] Fetching content for: ${title} from ${fetchUrl}`);
      ...
      try {
        ...
        const contentElement = doc.querySelector('.post-content') || doc.querySelector('article') || doc.querySelector('main');
...
function setupCloseHandlers() {
  ...
  heroCard.addEventListener('click', (e) => {
    if (e.target === heroCard) { // Only close if the click is directly on the backdrop
      window.closeHeroCard();
    }
  });
}
...
if (document.readyState !== 'loading') {
  console.log('[card-nav.js] Document already loaded, initializing immediately');
  initCardNavigation();
}