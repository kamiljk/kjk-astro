...
function detectHDRSupport() {
	...
	const currentAccent = document.body.style.getPropertyValue("--color-accent");
	const currentHover =
		document.body.style.getPropertyValue("--link-hover-color");
	const currentActive = document.body.style.getPropertyValue(
		"--link-active-color"
	);
	const currentHighlight =
		document.body.style.getPropertyValue("--highlight-color");
	...
	if (hasHDRSupport || hasP3Support) {
		...
		console.log(
			hasHDRSupport
				? "HDR display detected and enabled"
				: "P3 color gamut support detected"
		);
	} else {
	...
	if (currentAccent) {
		requestAnimationFrame(() => {
			...
			document.body.style.setProperty("--primary-color", currentAccent); // Legacy compatibility
			...
			document.body.style.setProperty(
				"--highlight-color",
				currentHighlight || currentAccent
			);
		});
