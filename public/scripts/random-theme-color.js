...
function generateHDRNeonColor() {
	...
	return {
		rgb: `rgb(${r}, ${g}, ${b})`,
		rgbValues: [r, g, b],
		p3: `color(display-p3 ${(r / 255).toFixed(3)} ${(g / 255).toFixed(3)} ${(
			b / 255
		).toFixed(3)})`,
	};
}
...
function generateColorVariants(color) {
	...
	return {
		hover: {
			rgb: `rgb(${Math.floor(hoverR)}, ${Math.floor(hoverG)}, ${Math.floor(
				hoverB
			)})`,
			p3: `color(display-p3 ${(hoverR / 255).toFixed(3)} ${(
				hoverG / 255
			).toFixed(3)} ${(hoverB / 255).toFixed(3)})`,
		},
		active: {
			rgb: `rgb(${Math.floor(activeR)}, ${Math.floor(activeG)}, ${Math.floor(
				activeB
			)})`,
			p3: `color(display-p3 ${(activeR / 255).toFixed(3)} ${(
				activeG / 255
			).toFixed(3)} ${(activeB / 255).toFixed(3)})`,
		},
	};
}
...
function saveAccentColor(color) {
	if (!color) {
		...
		const accentRgb = root.style.getPropertyValue("--color-accent-rgb");

		if (accentColor && accentRgb) {
			localStorage.setItem("accent-color", accentColor);
			localStorage.setItem("accent-color-rgb", accentRgb);
			return true;
		}
		return false;
	}
...
export function restoreAccentColor() {
	...
	if (savedColor && savedRgb) {
		...
		root.style.setProperty("--color-accent-rgb", savedRgb);

		// Also set legacy names for backwards compatibility
		...
		if (rgbValues.length === 3) {
			...
			// Set text color for accent backgrounds - calculate contrast-safe color
			const [r, g, b] = rgbValues;
			const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
			const accentTextColor = luminance > 0.5 ? "#18181b" : "#f7f7f8";
			root.style.setProperty("--color-accent-text", accentTextColor);

			if (supportsP3) {
				root.style.setProperty("--color-accent-hover", variants.hover.p3);
				root.style.setProperty("--color-accent-active", variants.active.p3);
				root.style.setProperty("--highlight-hover", variants.hover.p3);
				root.style.setProperty("--highlight-active", variants.active.p3);
			} else {
				root.style.setProperty("--color-accent-hover", variants.hover.rgb);
				root.style.setProperty("--color-accent-active", variants.active.rgb);
			...
			root.style.setProperty(
				"--color-accent-glow",
				`0 0 8px ${savedColor}, 0 0 16px ${savedColor}`
			);
		}
...
export function setRandomNeonAccent() {
	...
	// Set both standard and P3 CSS variables using the correct token names
	if (supportsP3) {
		root.style.setProperty("--color-accent", primaryColor.p3);
		root.style.setProperty("--color-accent-hover", variants.hover.p3);
		root.style.setProperty("--color-accent-active", variants.active.p3);
		...
		root.style.setProperty("--color-accent", primaryColor.rgb);
		root.style.setProperty("--color-accent-hover", variants.hover.rgb);
		root.style.setProperty("--color-accent-active", variants.active.rgb);
	...
	// Set text color for accent backgrounds - calculate contrast-safe color
	const [r, g, b] = primaryColor.rgbValues;
	const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
	const accentTextColor = luminance > 0.5 ? "#18181b" : "#f7f7f8"; // Dark text for light backgrounds, light text for dark
	root.style.setProperty("--color-accent-text", accentTextColor);

	// Set RGB values for effects like shadows, overlays, etc.
	root.style.setProperty(
		"--color-accent-rgb",
		primaryColor.rgbValues.join(", ")
	);
	root.style.setProperty(
		"--primary-color-rgb",
		primaryColor.rgbValues.join(", ")
	...
	// Generate glow effect
	const glowColor = supportsP3 ? primaryColor.p3 : primaryColor.rgb;
	root.style.setProperty(
		"--color-accent-glow",
	...
	console.log(
		`Applied random neon color: ${
			supportsP3 ? primaryColor.p3 : primaryColor.rgb
		}`
	);
	return primaryColor;
}
...
document.addEventListener("DOMContentLoaded", () => {
	// Always apply a new random neon color on every page load
	setRandomNeonAccent();
});
